...
<body>
	<%
		...
		if(!userID.equals(freeapp.getUserID())) {
			...
			if (request.getParameter("free_app_Title") == null || request.getParameter("free_app_Content") == null
					|| request.getParameter("free_app_Title").equals("") || request.getParameter("free_app_Content").equals("")) {
				PrintWriter script = response.getWriter();
				script.println("<script>");
				...
				int result = faDAO.update(Free_app_ID, request.getParameter("free_app_Title"), request.getParameter("free_app_Content"));
				if (result == -1) {
					PrintWriter script = response.getWriter();
					script.println("<script>");
